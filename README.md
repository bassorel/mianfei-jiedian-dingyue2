...
> A practical update stream for readers who only need the latest Clash article and a clean path to the current entry.
...
请打开上面的博客文章，在正文“免费订阅地址”区域获取当天可用入口。把当天文章当成唯一入口，更新节奏和失效说明就能放在同一页。
...
- 面向场景：Clash / Clash Verge Rev 免费节点订阅入口
- 推荐读者：使用 Clash 系客户端、需要快速导入和规则模式测试的读者
- 内容重点：Clash 订阅、代理组、规则模式、TUN 设置和导入失败排查；2026年9月13日更新；香港、日本、新加坡、美国 等地区；100 Mbps 公共限速
- 固定入口：https://free.tntyx.com/

## 今日导入路径

1. 打开今日文章，确认日期是 2026年9月13日，再获取正文里的免费订阅入口。
2. 优先使用 Clash 导入入口，或按站内 Clash Verge Rev 教程完成配置刷新；如果客户端没有弹出，就复制文章里的地址后手动新增订阅。
...
本站每日北京时间 00:00 自动处理免费节点用户：重置流量、刷新订阅 Token、写入服务端动态数据，并同步 GitHub 分享仓库。Clash 用户先看当天更新，再按导入教程走一遍最稳。 仓库里的 `sub/latest.txt` 只保存当天文章链接，不保存真实订阅地址。
